package com.fly.rabbitmq.exchange.fanout;
...
public class Receive01 {
    public static void main(String[] args) {
        ...
        try {
            ...
            /**
             * 由于fanout类型的交换机接收消息时类似于广播模式，它不需要绑定routingKey
             * 而又可能会有很多个消费者来接收这个交换机中的数据，因此我们创建队列时要创建一个速记的队列名称
             * 没有参数的queueDeclare方法会创建一个名字为随机的一个队列
             * 这个队列的数据非持久化
             * 排外的（同时只允许有一个消费者监听当前队列）
             * 自动删除的，没有任何消费者监听时会自动删除
             *
             * getQueue() 方法用来获取这个随机的队列名
             */
             ...
             *      由于是fanout类型的exchange不需要routingKey
             */
            ...
            channel.basicConsume(queueName, true, "", new DefaultConsumer(channel) {
                ...
                public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body) throws IOException {
                    ...
                    System.out.println("Receive01 消费者---> " + message);
                }
            });
        } catch (IOException e) {
            e.printStackTrace();
